...
- MODELEDITOR-012: the runtime Model Editor moved to its own repository, [XafModelEditor](https://github.com/MBrekhof/XafModelEditor), with its 133 unit tests, its E2E gate steps, a sample host of its own and its docs. It never depended on the builder, so nothing here changed but the removal: the add-on project, the twelve test files, the editor's gate block and helpers, its documentation and screenshots 13-17, the "Runtime Model Editor" section of `docs/api-notes.md` (a pointer now), and the sample host's five wiring lines. The entries below record what it did while it lived here.
- MODELEDITOR-011: "Customize layout" in the Model Editor opens the selected DetailView in a popup, built without a record, with XAF Blazor's own layout editor available over it: the nearest thing in Blazor to the WinForms Model Editor's designer.
- MODELEDITOR-015: "Drop my copy" in the Model Editor takes a user's own copy of a node out of their model differences when the model has a node of its own there, which is what a merge whose save of the user's record was refused leaves behind; Delete would have hidden the model's node behind a tombstone, and Reset node left an empty node of the user's own.
